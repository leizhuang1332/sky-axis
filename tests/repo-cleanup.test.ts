 ...
 *   - 清理 .sky-axis/repos/ 下符合「UUID 目录名 + .git/ + 未被引用」三条件的孤儿
...
async function makeClonedRepo(workspaceRoot: string, dirName: string): Promise<string> {
  const dir = join(workspaceRoot, '.sky-axis', 'repos', dirName)
...
describe('cleanupOrphanRepos', () => {
  ...
  it('.sky-axis/repos/ 不存在时返回空', async () => {
  ...
  it('清理符合全部三条件的 UUID 孤儿', async () => {
    ...
    const result = await cleanupOrphanRepos(workspaceRoot, new Set([`.sky-axis/repos/${'mini_harness'}`]))
    expect(result.removed).toEqual([`.sky-axis/repos/${'10831e16-462c-46eb-8732-a873cf43bb77'}`])
  ...
  it('不清理被 requirement 引用的目录(即使 UUID + .git/)', async () => {
    ...
    const result = await cleanupOrphanRepos(
      workspaceRoot,
      new Set([`.sky-axis/repos/${'10831e16-462c-46eb-8732-a873cf43bb77'}`]),
    )
    expect(result.removed).toEqual([])
    const { access } = await import('node:fs/promises')
  ...
  it('liveLocalPaths 为空集(无 requirement)→ 清掉所有 UUID 孤儿', async () => {
    ...
    expect(result.removed.sort()).toEqual([
      '.sky-axis/repos/11111111-2222-3333-4444-555555555555',
      '.sky-axis/repos/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee',
    ])
    expect(result.scanned).toBe(2)
  })
  ...
  it('混合场景:UUID 孤儿 + UUID 被引用 + 项目名 + 非 git —— 只删孤儿', async () => {
    ...
    const result = await cleanupOrphanRepos(
      workspaceRoot,
      new Set([`.sky-axis/repos/${'11111111-2222-3333-4444-555555555555'}`]),
    )
    expect(result.removed).toEqual([`.sky-axis/repos/${'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'}`])
    expect(result.scanned).toBe(4)

    const { access } = await import('node:fs/promises')
    await expect(access(orphan)).rejects.toThrow()
    await expect(access(join(workspaceRoot, '.sky-axis/repos/11111111-2222-3333-4444-555555555555'))).resolves.toBeUndefined()
    await expect(access(join(workspaceRoot, '.sky-axis/repos/mini_harness'))).resolves.toBeUndefined()
    await expect(access(join(workspaceRoot, '.sky-axis/repos/user-dir'))).resolves.toBeUndefined()
  })
})
