...
const ACTIVITIES: readonly ActivityItem[] = [
  { icon: '📌', text: '王哥给你分配了需求 #123',          time: '10 分钟前' },
  { icon: '🚀', text: '你提交了 abc1234：优化 session 加载', time: '1 小时前'  },
  { icon: '🐛', text: 'Bug 告警：内存泄漏，发生在 chat 主进程', time: '3 小时前' },
  { icon: '👀', text: '你审查了 MR #42：feat/login-sso',  time: '昨天'     },
  { icon: '✅', text: '小李合并了 MR #41：fix/memory-leak', time: '2 天前'   },
] as const
      ...
      <ul className={css.activityList}>
        {ACTIVITIES.map((a, idx) => (
          <li key={idx} className={css.activityItem}>
            <span className={css.activityItemIcon} aria-hidden="true">{a.icon}</span>
