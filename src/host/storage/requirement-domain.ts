 ...
 *
 * name: 'sky_axis_requirements'  —— 满足 UNIT_NAME_RE（^[a-z][a-z0-9_]*$）
 * version: 3                   —— Phase 2.5 完美主义演进：
 *                                   1) PrdFileSchema / AttachmentSchema 加 `path` required
 *                                   2) 物料 CRUD 路由（18 个）+ busboy multipart 上传
 *                                   3) 错误码加 'material-not-found'
 * tables.requirements: RequirementSchema —— KV 值校验（zod parse at boundary）
 *
 * ⚠️ 升级注意（DSH backend 物理约束）：
 *   - descriptor.version 与 medium 上已 stamp 的 version 不一致时
 *     **直接 reject `version-mismatch`**（不提供迁移 API）
 *   - 旧 v2 数据无法迁移到 v3 —— 遵循「项目未上线无包袱」原则，**不写迁移脚本**
 *   - dev 升级流程：
 *     1) 拉取最新代码（含 version: 3）
 *     2) 第一次启动会失败：DSH backend 拒绝 v2 medium，输出 `version-mismatch` 错误
 *     3) 删 storage 介质目录（DSH storage-json backend root）后重启
 *     4) 创建全新需求，物料数据走新结构
 *
 * 演进史：
 *   - v1：Phase 1 初始 schema
 *   - v2：Phase 2.1 物料 schema 从无到有 + 所有字段升级 required
 *   - v3：Phase 2.5 PrdFile / Attachment 加 path required（首次二进制文件落盘）
 */
