/**
 * SkyAxis ping/health 业务实现（host 半区）。
 *
...
import { SKY_AXIS_API_PREFIX } from '../protocol.ts'

/** 当前包名常量（host 半区硬编码，与 package.json 的 name 字段保持一致）。 */
const HOST_PACKAGE = '@leizhuang/sky-axis'

export class SkyAxisPingService {
  /** 进程启动时刻（用于 uptime 计算）。 */
